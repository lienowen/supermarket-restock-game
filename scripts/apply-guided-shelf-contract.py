...
scene_path = Path("src/game/presentation/scenes/StarterMarketScene.ts")
scene = scene_path.read_text()
scene = replace_once(scene, '''    this.controller = new RestockSceneController({
...
''', "scene controller pacing")
scene = replace_once(scene, '''    const rushTuning = context.campaignLevel.level.tuning.rush;
...
''', '''    this.rush = new RestockRushController({
      ...
      itemsPerRow,
''', "rush pacing")
scene = replace_once(scene, '''        : context.campaignLevel.level.tuning.rush?.timeoutEnabled === false
...
''', '''        : context.campaignLevel.level.tuning.rush?.timeoutEnabled === false
          ...
          : "FIND THE GLOWING SHELF · STOCK 3 ITEMS"
''', "guided meter copy")
scene = replace_once(scene, '''    const itemLabel = `${result.stockedItemCount}/${result.snapshot.itemsPerRow}`;
...
audit_path = Path("scripts/capture-release-regressions.mjs")
audit = audit_path.read_text()
audit = replace_once(audit, '''const restock = (number, id, label, initial, complete) => Object.freeze({
...
''', "restock audit level contract")
audit = replace_once(audit, '''  restock(1, "starter-level-001", "Cola first delivery",
    { coins: 100, stars: 0 },
...
''', '''  restock(1, "starter-level-001", "Cola first delivery",
    ...
    1),
''', "guided audit pacing")
audit = replace_once(audit, '''    movementRequired: false,
...
''', "audit assertions")
audit = replace_once(audit, '''      report.regressions.productionAssetRuntime = (
...
start = audit.index("async function completeRestock(")
end = audit.index("\nasync function completeCheckout", start)
audit = audit[:start] + '''async function completeRestock(page, level, detailed) {
...
''' + audit[end:]
old_wait = '''async function waitRowCount(page, rowIndex, count) {
...
'''
new_wait = '''async function waitRowCount(page, rowIndex, logicalCount, physicalCount) {
...
'''
audit = replace_once(audit, old_wait, new_wait, "physical shelf waiter")
audit_path.write_text(audit)


workflow_path = Path(".github/workflows/ui-audit.yml")
workflow = workflow_path.read_text()
workflow = replace_once(
    workflow,
    '              context: passed ? "UI Regression Audit" : `UI Regression Audit (${failureKey})`,\n',
    '              context: "UI Regression Audit",\n',
    "stable audit status context"
)
