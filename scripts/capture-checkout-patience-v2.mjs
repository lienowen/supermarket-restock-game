...
try {
  ...
  // Use the real flow: scan first, then make two wrong weighing decisions.
  // Two penalties leave enough time to observe the impatient state without the
  // audit itself racing the customer to zero patience.
  ...
  // Fresh session for the completion gate. Keep it fast and deterministic.
  ...
  if (failed.length) throw new Error(`Mature Level 7 audit v2 failed: ${failed.join(", ")}`);
} catch (error) {
  thrownError = error;
