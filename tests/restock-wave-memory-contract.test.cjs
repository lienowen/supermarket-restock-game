...
test("Level 10 switches to a readable cooler close-up for the 18 placements", () => {
  ...
  assert.match(closeupSource, /const CLOSEUP_WIDTH = 1600/);
  assert.match(closeupSource, /const CLOSEUP_HEIGHT = 900/);
  assert.match(closeupSource, /const SLOT_WIDTH = 434/);
  assert.match(closeupSource, /const SLOT_XS = \[560, 1062\]/);
  assert.match(closeupSource, /const SLOT_YS = \[263, 425, 588\]/);
